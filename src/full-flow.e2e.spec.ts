...
class AdapterMock {
  ...
  private workspaces = new Map<number, any[]>();
  ...
  registerUser(email: string, name: string, password: string) {
    ...
    const u = { id: this.uid++, email, name, password };
  ...
  createProject(userId: number, workspaceId: number, payload: any) {
    const project = {
      id: this.pid++,
      workspaceId,
      ...payload,
  ...
  getProjectBoardIssues(userId: number, projectId: number) {
    return [{ id: 1, title: 'Issue 1', projectId }];
  }
}
